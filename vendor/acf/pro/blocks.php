...
function acf_register_block_type( $block ) {
    ...
    // Require name.
    if( !$block['name'] ) {
	    $message = __( 'Block type name is required.', 'plusdin-homepage-generator' );
	...
	if( acf_has_block_type($block['name']) ) {
		$message = sprintf( __( 'Block type "%s" is already registered.', 'plusdin-homepage-generator' ), $block['name'] );
		_doing_it_wrong( __FUNCTION__, $message, '5.8.0' );
		return false;
	}
...
function acf_enqueue_block_assets() {
	...
	acf_localize_text(array(
		'Switch to Edit'		=> __('Switch to Edit', 'plusdin-homepage-generator'),
		'Switch to Preview'		=> __('Switch to Preview', 'plusdin-homepage-generator'),
		'Change content alignment'	=> __('Change content alignment', 'plusdin-homepage-generator'),

		/* translators: %s: Block type title */
		'%s settings'			=> __('%s settings', 'plusdin-homepage-generator'),
	));
