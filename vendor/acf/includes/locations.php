...
function acf_register_location_type( $class_name ) {
	...
	if( !class_exists($class_name) ) {
		$message = sprintf( __( 'Class "%s" does not exist.', 'plusdin-homepage-generator' ), $class_name );
	...
	if( $store->has( $name ) ) {
		$message = sprintf( __( 'Location type "%s" is already registered.', 'plusdin-homepage-generator' ), $name );
		_doing_it_wrong( __FUNCTION__, $message, '5.9.0' );
		return false;
	}
...
function acf_get_location_rule_types() {
	...
	// Default categories.
	$categories = array(
		'post'		=> __('Post', 'plusdin-homepage-generator'),
		'page'		=> __('Page', 'plusdin-homepage-generator'),
		'user'		=> __('User', 'plusdin-homepage-generator'),
		'forms'		=> __('Forms', 'plusdin-homepage-generator'),
	);
