...
class ACF_Field_User extends ACF_Field {
	...
	function initialize() {
		...
		// Props.
		$this->name = 'user';
		$this->label = __("User",'plusdin-homepage-generator');
	...
	function render_field_settings( $field ) {
		...
		acf_render_field_setting( $field, array(
			'label'			=> __('Filter by role','plusdin-homepage-generator'),
			'instructions'	=> '',
			'type'			=> 'select',
			'name'			=> 'role',
			...
			'ui'			=> 1,
			'allow_null'	=> 1,
			'placeholder'	=> __("All user roles",'plusdin-homepage-generator'),
		));
		...
		acf_render_field_setting( $field, array(
			...
			'name'			=> 'return_format',
			'choices'		=> array(
				'array'			=> __("User Array",'plusdin-homepage-generator'),
				'object'		=> __("User Object",'plusdin-homepage-generator'),
				'id'			=> __("User ID",'plusdin-homepage-generator'),
			),
			'layout'	=>	'horizontal',
		));
	}
	...
	function ajax_query_init( $request, $query ) {
		...
		// Require field.
		if( !$query->field ) {
			$query->send( new WP_Error( 'acf_missing_field', __( 'Error loading field.', 'plusdin-homepage-generator' ), array( 'status' => 404 ) ) );
		}
	}
