...
class acf_field_taxonomy extends acf_field {
	...
	*/
	...
	function initialize() {
		...
		// vars
		$this->name = 'taxonomy';
		$this->label = __("Taxonomy",'plusdin-homepage-generator');
...
<div <?php acf_esc_attr_e($div); ?>>
	...
	*/
	...
	function render_field_checkbox( $field ) {
		...
		$args = array(
			'taxonomy'     		=> $field['taxonomy'],
			'show_option_none'	=> sprintf( _x('No %s', 'No terms', 'plusdin-homepage-generator'), strtolower($taxonomy_obj->labels->name) ),
			'hide_empty'   		=> false,
			'style'        		=> 'none',
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		acf_render_field_setting( $field, array(
			'label'			=> __('Taxonomy','plusdin-homepage-generator'),
			'instructions'	=> __('Select the taxonomy to be displayed','plusdin-homepage-generator'),
			'type'			=> 'select',
			'name'			=> 'taxonomy',
		...
		// field_type
		acf_render_field_setting( $field, array(
			'label'			=> __('Appearance','plusdin-homepage-generator'),
			'instructions'	=> __('Select the appearance of this field','plusdin-homepage-generator'),
			'type'			=> 'select',
			'name'			=> 'field_type',
			'optgroup'		=> true,
			'choices'		=> array(
				__("Multiple Values",'plusdin-homepage-generator') => array(
					'checkbox' => __('Checkbox', 'plusdin-homepage-generator'),
					'multi_select' => __('Multi Select', 'plusdin-homepage-generator')
				),
				__("Single Value",'plusdin-homepage-generator') => array(
					'radio' => __('Radio Buttons', 'plusdin-homepage-generator'),
					'select' => _x('Select', 'noun', 'plusdin-homepage-generator')
				)
			)
		));
		...
		// add_term
		acf_render_field_setting( $field, array(
			'label'			=> __('Create Terms','plusdin-homepage-generator'),
			'instructions'	=> __('Allow new terms to be created whilst editing','plusdin-homepage-generator'),
			'name'			=> 'add_term',
			'type'			=> 'true_false',
			'ui'			=> 1,
		));
		...
		// save_terms
		acf_render_field_setting( $field, array(
			'label'			=> __('Save Terms','plusdin-homepage-generator'),
			'instructions'	=> __('Connect selected terms to the post','plusdin-homepage-generator'),
			'name'			=> 'save_terms',
			'type'			=> 'true_false',
			'ui'			=> 1,
		));
		...
		// load_terms
		acf_render_field_setting( $field, array(
			'label'			=> __('Load Terms','plusdin-homepage-generator'),
			'instructions'	=> __('Load value from posts terms','plusdin-homepage-generator'),
			'name'			=> 'load_terms',
			'type'			=> 'true_false',
			'ui'			=> 1,
		));
		...
		// return_format
		acf_render_field_setting( $field, array(
			'label'			=> __('Return Value','plusdin-homepage-generator'),
			'instructions'	=> '',
			'type'			=> 'radio',
			'name'			=> 'return_format',
			'choices'		=> array(
				'object'		=>	__("Term Object",'plusdin-homepage-generator'),
				'id'			=>	__("Term ID",'plusdin-homepage-generator')
			),
			'layout'	=>	'horizontal',
		));
	...
	function ajax_add_term() {
		...
		if( !current_user_can( $taxonomy_obj->cap->manage_terms) ) {
			wp_send_json_error(array(
				'error'	=> sprintf( __('User unable to add new %s', 'plusdin-homepage-generator'), $taxonomy_label )
			));
		}
		...
		if( $args['term_name'] ) {
			...
			if( term_exists($args['term_name'], $field['taxonomy'], $args['term_parent']) ) {
				wp_send_json_error(array(
					'error'	=> sprintf( __('%s already exists', 'plusdin-homepage-generator'), $taxonomy_label )
				));
			}
			...
			wp_send_json_success(array(
				'message'		=> sprintf( __('%s added', 'plusdin-homepage-generator'), $taxonomy_label ),
		...
		acf_render_field_wrap(array(
			'label'			=> __('Name', 'plusdin-homepage-generator'),
			'name'			=> 'term_name',
			'type'			=> 'text'
		));
		...
		if( is_taxonomy_hierarchical( $field['taxonomy'] ) ) {
			...
			acf_render_field_wrap(array(
				'label'			=> __('Parent', 'plusdin-homepage-generator'),
				'name'			=> 'term_parent',
				'type'			=> 'select',
				'allow_null'	=> 1,
				'ui'			=> 0,
		...
		?><p class="acf-submit">
			<button class="acf-submit-button button button-primary" type="submit"><?php _e("Add", 'plusdin-homepage-generator'); ?></button>
		</p>
		</form><?php
