...
function acf_validate_save_post( $show_errors = false ) {
	...
	if( $show_errors ) {
		...
		$message = '<h2>' . __('Validation failed', 'plusdin-homepage-generator') . '</h2>';
		$message .= '<ul>';
		...
		// die
		wp_die( $message, __('Validation failed', 'plusdin-homepage-generator') );
...
function acf_validate_value( $value, $field, $input ) {
	...
	// vars
	$valid = true;
	$message = sprintf( __( '%s value is required', 'plusdin-homepage-generator' ), $field['label'] );
