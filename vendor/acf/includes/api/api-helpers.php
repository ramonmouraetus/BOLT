...
function acf_get_image_sizes() {
	...
	// vars
	$sizes = array(
		'thumbnail'	=>	__("Thumbnail",'plusdin-homepage-generator'),
		'medium'	=>	__("Medium",'plusdin-homepage-generator'),
		'large'		=>	__("Large",'plusdin-homepage-generator')
	);
	...
	// add full end
	$sizes['full'] = __("Full Size",'plusdin-homepage-generator');
...
function acf_get_post_title( $post = 0, $is_search = false ) {
	...
	if( $title === '' ) {
		...
		$title = __('(no title)', 'plusdin-homepage-generator');
...
function acf_validate_attachment( $attachment, $field, $context = 'prepare' ) {
	...
	if( $file['width'] || $file['height'] ) {
		...
		if( $file['width'] ) {
			...
			if( $min_width && $file['width'] < $min_width ) {
				...
				// min width
				$errors['min_width'] = sprintf(__('Image width must be at least %dpx.', 'plusdin-homepage-generator'), $min_width );
				
			} elseif( $max_width && $file['width'] > $max_width ) {
				
				// min width
				$errors['max_width'] = sprintf(__('Image width must not exceed %dpx.', 'plusdin-homepage-generator'), $max_width );
		...
		if( $file['height'] ) {
			...
			if( $min_height && $file['height'] < $min_height ) {
				...
				// min height
				$errors['min_height'] = sprintf(__('Image height must be at least %dpx.', 'plusdin-homepage-generator'), $min_height );
				
			}  elseif( $max_height && $file['height'] > $max_height ) {
				
				// min height
				$errors['max_height'] = sprintf(__('Image height must not exceed %dpx.', 'plusdin-homepage-generator'), $max_height );
	...
	if( $file['size'] ) {
		...
		if( $min_size && $file['size'] < acf_get_filesize($min_size) ) {
			...
			// min width
			$errors['min_size'] = sprintf(__('File size must be at least %s.', 'plusdin-homepage-generator'), acf_format_filesize($min_size) );
			
		} elseif( $max_size && $file['size'] > acf_get_filesize($max_size) ) {
				
			// min width
			$errors['max_size'] = sprintf(__('File size must not exceed %s.', 'plusdin-homepage-generator'), acf_format_filesize($max_size) );
	...
	if( $file['type'] ) {
		...
		if( !empty($mime_types) && !in_array($file['type'], $mime_types) ) {
			...
			if( count($mime_types) > 1 ) {
				...
				$mime_types[] = $last2 . ' ' . __('or', 'plusdin-homepage-generator') . ' ' . $last1;
				
			}
			
			$errors['mime_types'] = sprintf(__('File type must be %s.', 'plusdin-homepage-generator'), implode(', ', $mime_types) );
...
function acf_translate( $string ) {
	...
	// vars
	return __( $string, $textdomain, 'plusdin-homepage-generator' );
